...
watch(() => props.rows, (val) => {
    if (val?.length) {
        // giữ nguyên lựa chọn nếu vẫn tồn tại; nếu không thì pick cái đầu
...
const seriesDaily = computed(() => {
    ...
    for (const r of (props.rows || [])) {
        ...
        const t = toNum(r.tong_slsp_nang_suat)
        const s = toNum(r.ns_chuan_ngay)
        total.push(t || null)
        kpi.push(s || null)
...
function comboOption() {
    const { cats, total, kpi, ratioPct } = seriesDaily.value
    return {
        tooltip: {
            trigger: 'axis',
...
/* ====== Pie (so với 100%) cho team đang chọn ====== */
function pieOption() {
    const r = currentTeam.value
    const teamLabel = r ? `${r.workshop_name ?? ''} - ${r.team_label ?? r.team_id}` : 'Chưa có dữ liệu'
    const total = toNum(r?.tong_slsp_nang_suat)
    const kpi = toNum(r?.ns_chuan_ngay)

    // Không có KPI hợp lệ -> trả về placeholder
    if (kpi <= 0) {
        return {
    ...
    // Tỷ lệ thật (không giới hạn)
    const ratio = (total / kpi) * 100
    const achieved = Math.min(ratio, 100)       // Đạt tối đa 100%
    const shortage = Math.max(100 - ratio, 0)   // Thiếu (nếu <100)
    const over = Math.max(ratio - 100, 0)       // Vượt (nếu >100)
    const allZero = !achieved && !shortage && !over
    ...
    return {
        ...
        tooltip: {
            trigger: 'item',
            valueFormatter: (v) => `${Number(v).toFixed(2)}%`,  // làm tròn 2 số sau dấu phẩy
        },
        legend: { bottom: 0, left: 'center' },
        series: [
            {
                ...
                radius: ['45%', '70%'],
                center: ['50%', '50%'],
                // Hiển thị giá trị thật của từng lát (không phải % theo tổng lát)
                label: {
                    show: true,
        ...
        // Text giữa vòng tròn – cũng làm tròn 2 chữ số
        graphic: [
            {
                type: 'text',
                left: 'center',
