    ...
    <div v-if="isMobile" class="mobile-card-list">
        <a-card v-for="(record, index) in data" :key="record.id" class="mobile-card" size="small">
            ...
            <div class="card-content">
                <div class="info-row">
                    <span class="label">Mã tổ:</span>
                    <span class="value">{{ record.code }}</span>
                </div>
                <div class="info-row">
                    <span class="label">Thuộc xưởng:</span>
                    <span class="value">{{ record?.workshop?.name || record?.workshop_name || '' }}</span>
                </div>
            </div>
        </a-card>
        ...
        <div class="mobile-pagination mt-4" v-if="pagination && pagination.total > pagination.pageSize">
...
const columns = [
    { title: 'STT', key: 'stt', width: 80, customRender: ({ index }) => getSTT(index) },
