<template>
    <a-modal v-model:visible="innerVisible" :title="isEdit ? 'Cập nhật xưởng' : 'Thêm xưởng'" @ok="submit"
        @cancel="$emit('cancel')" ok-text="Lưu" cancel-text="Huỷ" destroyOnClose>
        <a-form :model="form" layout="vertical">
            <a-form-item label="Tên xưởng" required>
                <a-input v-model:value="form.name" />
            </a-form-item>
            <a-form-item label="Mã xưởng" required>
