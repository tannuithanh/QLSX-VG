<template>
  <a-modal v-model:visible="innerVisible" title="Import lỗi sản xuất" ok-text="Nhập" cancel-text="Đóng"
    :ok-button-props="{ disabled: !payload.length || submitting, loading: submitting }" :width="'90vw'"
    :mask-closable="false" destroy-on-close wrap-class-name="import-clean" @ok="confirm"
...
function processWorkbook(wb) {
  ...
  for (let r = 0; r < Math.min(5, headRows.length); r++) {
    ...
    for (const list of Object.values(REQ)) {
      if (list.some(a => line.includes(a))) hit++
    }
    if (hit >= 4) {
      headerRowIdx = r
      break
  ...
  const headers = merged.map((txt, i) => ({
    title: txt || `COL_${i + 1}`,
    key: slugHeader(txt) || `col_${i + 1}`
  }))
  ...
  rows.forEach(r => {
    ...
    else if (r.__dateISO > currentYmd) e.push('Ngày sản xuất không được lớn hơn ngày hiện tại.')
    ...
    for (let i = 1; i <= 3; i++) {
      ...
      const stKey = viSlug(rawStage)
      const cdKey = codeKey(rawCode)
...
const wsCache = ref([])
const teamCache = ref([])
const errorCodeSet = ref(new Set())
const stageCodeSet = ref(new Set())

async function ensureMastersLoaded() {
  const jobs = []

  if (!wsCache.value.length) {
    jobs.push(workshopApi.listAll().then(v => { wsCache.value = Array.isArray(v) ? v : [] }))
  }
  if (!teamCache.value.length) {
    jobs.push(teamApi.listAll().then(v => { teamCache.value = Array.isArray(v) ? v : [] }))
  }
  if (!errorCodeSet.value.size) {
    jobs.push(errorCodeApi.listAll().then(v => {
      errorCodeSet.value = new Set((Array.isArray(v) ? v : []).map(x => codeKey(x.code)))
    }))
  }
  if (!stageCodeSet.value.size) {
    jobs.push(stageApi.listAll().then(v => {
      stageCodeSet.value = new Set((Array.isArray(v) ? v : []).map(x => codeKey(x.code)))
    }))
  }
...
async function precheckAll() {
  ...
  for (const r of payload.value) {
    ...
    if (!r.production_date) {
      rowErrs.push('Thiếu "Ngày sản xuất".')
    } else if (r.production_date > currentYmd) {
      rowErrs.push('Ngày sản xuất không được lớn hơn ngày hiện tại.')
    }

    const ws = findWorkshop(r.workshop)
    if (!ws) {
      rowErrs.push(`Không tìm thấy Xưởng "${r.workshop}" trong hệ thống.`)
    }

    let team = null
    if (ws) {
      team = findTeam(r.team, ws.id)
      if (!team) {
        rowErrs.push(`Không tìm thấy Tổ/nhóm "${r.team}" thuộc Xưởng ${fmtWs(ws)}.`)
      }
    }
    ...
    if (m1 && !errorCodeSet.value.has(codeKey(m1))) rowErrs.push(`Mã lỗi 1 "${m1}" không tồn tại`)
    if (s1 && !stageCodeSet.value.has(codeKey(s1))) rowErrs.push(`Công đoạn PS 1 "${s1}" không tồn tại`)
    if (m2 && !errorCodeSet.value.has(codeKey(m2))) rowErrs.push(`Mã lỗi 2 "${m2}" không tồn tại`)
    if (s2 && !stageCodeSet.value.has(codeKey(s2))) rowErrs.push(`Công đoạn PS 2 "${s2}" không tồn tại`)
    if (m3 && !errorCodeSet.value.has(codeKey(m3))) rowErrs.push(`Mã lỗi 3 "${m3}" không tồn tại`)
    if (s3 && !stageCodeSet.value.has(codeKey(s3))) rowErrs.push(`Công đoạn PS 3 "${s3}" không tồn tại`)

    if (rowErrs.length) {
      errs.push({ row: r.__stt, errors: rowErrs })
    }
  }
...
async function confirm() {
  ...
  try {
    ...
    if (!pre.ok) {
      ...
      success.value = false
      message.error('Có dòng không hợp lệ. Vui lòng kiểm tra tab Lỗi.')
      return
    }
