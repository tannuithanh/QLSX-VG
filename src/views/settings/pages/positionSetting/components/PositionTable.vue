<template>
    <a-table :columns="columns" :data-source="rows" :loading="loading" :pagination="pagination" :row-key="r => r.id"
        :scroll="{ x: 600 }" size="middle" @change="onChange">
        <template #bodyCell="{ column, record, index }">
            ...
            <template v-else-if="column.key === 'action'">
                <a-space>
                    ...
                    <a-tooltip title="Xoá">
                        <a-popconfirm title="Xoá chức vụ này?" ok-text="Xoá" ok-type="danger" cancel-text="Huỷ"
...
const columns = [
    ...
    { title: 'Tên chức vụ', dataIndex: 'name', width: 300 },
